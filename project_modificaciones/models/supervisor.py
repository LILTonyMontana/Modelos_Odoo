...
class SupervisorArea(models.Model):
    ...
    name = fields.Char(string='Supervisor', required=True)
    cliente = fields.Many2one('res.partner', string='Contacto', required=True)

class PlantaAvance(models.Model):
    _name = 'planta.avance'
    _description = 'Planta'
...
class CentroTrabajoAvance(models.Model):
    _name = 'centro.trabajo.avance'
    ...
    def create(self, vals):
        vals['color'] = randint(0, 9)
        return super(CentroTrabajoAvance, self).create(vals)

class ResPartner(models.Model):
    _inherit = 'res.partner'

    centros_trabajo_ids = fields.One2many(
...
class Disciplina(models.Model):
    ...
    sub_update_id = fields.Many2one(
        "project.sub.update", string="Creación De Avances"
    )
